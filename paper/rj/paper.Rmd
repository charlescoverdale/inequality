...
Every inequality function in the package accepts \code{ci = TRUE}
together with \code{R} and \code{level} arguments. The bootstrap
resamples the data with replacement (with probability proportional to
the survey weights, so the weighting flows through to the variance) and
takes percentile-method intervals. The Gini additionally exposes a
closed-form jackknife variance derived in @davidson2009reliable via
\code{method = "asymptotic"}, which is cheap in large samples; the
...
\code{iq\_compare()} runs a single bootstrap loop and reports a CI for
...
1. \CRANpkg{inequality} is a measurement package, not a survey-design
   package. It accepts survey weights and propagates them through the
   bootstrap via probability-proportional resampling, but it does not
   compute replicate-weight or Taylor-series variance estimators for
   complex survey designs with clustering or stratification. Users
   needing design-based inference should pair these functions with
   \CRANpkg{survey} or \CRANpkg{srvyr}.
...
3. The package does not reconcile survey income to national accounts,
...
4. All functions are pure R. For panel inequality analysis at the scale
...
Confidence intervals are now exposed uniformly across every inequality
function as of `inequality` 0.2.0, in response to feedback from Cowell
and Flachaire (personal communication, 2026). Each function takes
`ci`, `R`, and `level` arguments; `iq_compare()` runs a single bootstrap
loop and propagates CIs to every row of the comparison table.
